    ...
    public static Task<string> DiscoverApps(CancellationToken ct = default)
    {
        ...
        foreach (var app in apps)
        {
            ...
            if (obj.TryGetPropertyValue("protocolVersion", out var versionNode))
            {
                ...
                if (appVersion != "0.4.0")
                    obj["versionWarning"] = $"App uses protocol v{appVersion}, CLI tool is v0.4.0. Some tools may not work. Update AvaloniaMcp.Diagnostics to match.";
            }
            else
            {
                obj["versionWarning"] = "App does not report protocol version. It may be using an older AvaloniaMcp.Diagnostics (<0.4.0). Update the package.";
            }
